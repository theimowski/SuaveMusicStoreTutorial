...
Suave applications can be hosted as a standalone Console Application. 
...
Note there is no `Main` method defined in `App.fs` - what happens here is that the `startWebServer` function is invoked immediately after the program is run and Suave starts listening for incoming requests until the process is killed.
